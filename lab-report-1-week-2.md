...
`scp WhereAmI.java username@ieng6.ucsd.edu:~/; ssh username@ieng6.ucsd.edu "javac WhereAmI.java; java WhereAmI"` command allows the user to secure copy the file safely to the virtual machine with no need to enter the password repeatedly. As a result, using the SSH key is a helpful way to save users time when processing files via virtual machine.
